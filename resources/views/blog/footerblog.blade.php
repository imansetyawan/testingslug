<footer class="footer-widget-section">
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <aside class="footer-widget">
                    <div class="about-img"><img src="{{ url('asset/dist2/images/footer-logo.png')}}" alt="Kotha"></div>
                    
                    <div class="address">
                        <h4 class="text-uppercase">contact Info</h4>
                        @foreach ($infos as $info)
                        <i class="fa fa-home"></i><span> {{$info->alamat}}</span><br>
                        <i class="fa fa-phone"></i><span> {{$info->telp}}</span><br>
                        <i class="fa fa-envelope"></i><span> {{$info->email}}</span><br>
                        @endforeach
                    </div>
                </aside>
            </div>

            <div class="col-md-4">
               <!--  <aside class="footer-widget">
                    <h3 class="widget-title text-uppercase">Testimonials</h3>

                    <div id="myCarousel" class="carousel slide" data-ride="carousel"> -->
                        <!--Indicator-->
                        <!-- <ol class="carousel-indicators">
                            <li data-target="#myCarousel" data-slide-to="0" class="active"></li>
                            <li data-target="#myCarousel" data-slide-to="1"></li>
                            <li data-target="#myCarousel" data-slide-to="2"></li>
                        </ol>
                        <div class="carousel-inner" role="listbox">
                            <div class="item active">
                                <div class="single-review">
                                    <div class="review-text">
                                        <p>Lorem ipsum dolor sit amet, conssadipscing elitr, sed diam nonumy eirmod
                                            tempvidunt ut labore et dolore magna aliquyam erat,sed diam voluptua. At
                                            vero eos et accusam justo duo dolores et ea rebum.gubergren no sea takimata
                                            magna aliquyam eratma</p>
                                    </div>
                                    <div class="author-id">
                                        <img src="{{ url('asset/dist2/images/author.png')}}" alt="">

                                        <div class="author-text">
                                            <h4>Sophia</h4>

                                            <h4>CEO, ReadyTheme</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="item">
                                <div class="single-review">
                                    <div class="review-text">
                                        <p>Lorem ipsum dolor sit amet, conssadipscing elitr, sed diam nonumy eirmod
                                            tempvidunt ut labore et dolore magna aliquyam erat,sed diam voluptua. At
                                            vero eos et accusam justo duo dolores et ea rebum.gubergren no sea takimata
                                            magna aliquyam eratma</p>
                                    </div>
                                    <div class="author-id">
                                        <img src="http://localhost/laravel/dist2/images/author.png" alt="">

                                        <div class="author-text">
                                            <h4>Sophia</h4>

                                            <h4>CEO, ReadyTheme</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="item">
                                <div class="single-review">
                                    <div class="review-text">
                                        <p>Lorem ipsum dolor sit amet, conssadipscing elitr, sed diam nonumy eirmod
                                            tempvidunt ut labore et dolore magna aliquyam erat,sed diam voluptua. At
                                            vero eos et accusam justo duo dolores et ea rebum.gubergren no sea takimata
                                            magna aliquyam eratma</p>
                                    </div>
                                    <div class="author-id">
                                        <img src="http://localhost/laravel/dist2/images/author.png" alt="">

                                        <div class="author-text">
                                            <h4>Sophia</h4>

                                            <h4>CEO, ReadyTheme</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                </aside> -->
            </div>
            <div class="col-md-4">
                <aside class="footer-widget">
                    <h3 class="widget-title text-uppercase">Random Artikel</h3>


                    <div class="custom-post">
                        @foreach($randoms as $random)
                        <div>
                            <a href="{{ route('single_artikel', ['slug' => $random->slug])  }}"><img width="295" height="190" src="{{asset('image/thumb_'.$random->image)}}" alt=""></a>
                        </div>
                        <div>
                            <a href="{{ route('single_artikel', ['slug' => $random->slug])  }}" class="text-uppercase">{{$random->judulartikel}}</a>
                            <span class="p-date">{{ date('d F Y', strtotime($random->created_at))}}</span>
                        </div>
                        @endforeach
                    </div>
                </aside>
            </div>
        </div>
    </div>
    <div class="footer-copy">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="text-center">&copy; 2015 <a href="">Kotha PRO, </a>,Designed with <i
                            class="fa fa-heart"></i>
                        by <a href="#">ShapedTheme</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>
</body>